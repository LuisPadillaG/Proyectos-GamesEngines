...
public class Jugador : MonoBehaviour
{
    ...
    [SerializeField] GameObject prefabVengefulSpirit;
    ...
    void Update()
    {
        ...
        {
            contadorCaidavengativo = 0.2f;
            
            Instantiate(prefabVengefulSpirit, this.transform.position + Vector3.up * 0.5f, this.transform.rotation);
        }
        if(contadorCaidavengativo > 0) 
        {
            velocidad.y = 0;
            contadorCaidavengativo -= Time.deltaTime;
        }
        ...
        if (Physics.Raycast(this.transform.position + Vector3.up * 0.5f, Vector3.left, 0.35f) && is_graunded <= 0)
        {
            if (estadoPared == 0) {
                if (velocidad.y < 0)
                {
        ...
        {
            velocidad.x = velocidadParedX;
            velocidadParedX -= 20 * Time.deltaTime;
        }
